...
const telegramTokenPattern = /^\d{6,20}:[A-Za-z0-9_-]{20,}$/;
...
export const telegramSettingsSchema = z
  .object({
    ...
    botToken: z
      .unknown()
      .transform((value) => normalizeOptionalString(value))
      .refine((value) => !value || value.length <= 260, "توکن بات تلگرام بیش از حد طولانی است.")
      .refine((value) => !value || telegramTokenPattern.test(value), "توکن بات تلگرام معتبر نیست."),
    chatId: z
      .unknown()
      .transform((value) => normalizeOptionalString(value))
