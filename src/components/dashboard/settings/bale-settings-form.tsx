...
import {
  discoverBaleChatAction,
  fetchBaleRecentChatsAction,
  saveBaleSettingsAction,
  selectBaleDiscoveredChatAction,
  sendBaleTestMessageAction,
} from "@/lib/actions/bale-settings-actions";
import { initialBaleSettingsActionState } from "@/lib/actions/bale-settings-state";
import { toPersianDigits } from "@/lib/date/jalali";

export type BaleSettingsFormValues = {
  isEnabled: boolean;
...
type BaleSettingsFormProps = {
  setting: BaleSettingsFormValues | null;
  canSendTest: boolean;
...
const eventToggles = [
  ...
  },
  {
    ...
    title: "گزارش روزانه",
    description: "خلاصه عملکرد روزانه در بله",
    defaultChecked: false,
  },
  {
...
function getToggleDefault(setting: BaleSettingsFormValues | null, name: string, fallback: boolean) {
  if (!setting) {
    return fallback;
  }

  return Boolean(setting[name as keyof BaleSettingsFormValues]);
}
...
export function BaleSettingsForm({
  setting,
  canSendTest,
  testDisabledReason,
}: BaleSettingsFormProps) {
  const tokenRef = useRef<HTMLInputElement>(null);
  const [saveState, saveAction, isSaving] = useActionState(
    saveBaleSettingsAction,
    initialBaleSettingsActionState,
  );
      ...
      <div className="grid gap-5">
        <section className="rounded-[1.75rem] border border-[#d8c08b]/62 bg-[#fff9ee]/94 p-4 shadow-[0_18px_60px_rgba(17,24,39,0.08)] sm:rounded-[2rem] sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              <p className="text-xs font-black text-[#9f7131]">توکن بازو</p>
              <h2 className="mt-2 text-2xl font-black text-[#111827]">
                توکن بازوی بله
              </h2>
              <p className="mt-2 text-sm font-bold leading-7 text-[#6d5f49]">
                اگر می‌خواهید بازو را تغییر دهید، توکن جدید را وارد کنید؛ در غیر
            ...
            >
              ...
              {hasToken ? "توکن بازو ثبت شده است" : "نیازمند ثبت توکن"}
            </span>
          </div>
        ...
        <section className="rounded-[1.75rem] border border-[#d8c08b]/62 bg-[#fff9ee]/94 p-4 shadow-[0_18px_60px_rgba(17,24,39,0.08)] sm:rounded-[2rem] sm:p-6">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-black text-[#111827]">
                اتصال گفتگو یا کانال بله
              </h2>
              <p className="mt-2 text-sm font-bold leading-7 text-[#6d5f49]">
                لینک عمومی، نام کاربری یا شناسه عددی گفتگو را وارد کنید. اگر دریافت هوشمند به‌خاطر فیلترینگ یا قطع دسترسی بله جواب نداد، همان مقدار به‌صورت دستی قابل ذخیره است.
              </p>
            </div>
            <span
              className={
          ...
          <div className="mt-5 grid gap-4">
            <form action={discoverAction} className="grid gap-3 md:grid-cols-[1fr_auto]">
              <Field
                ...
                name="chatLookup"
                placeholder="مثلاً @my_channel یا ble.ir/my_channel یا -1001234567890"
                dir="ltr"
              />
              <button
                type="submit"
            ...
            <div className="rounded-2xl border border-[#e4c98a] bg-[#fff7e6]/80 p-3 text-xs font-black leading-6 text-[#7a4a12]">
              اگر پیام «زمان مناسب دریافت نشد» دیدید، مشکل معمولاً از دسترسی سرور برنامه به Bale Bot API است. می‌توانید مقدار نرمال‌شده مثل <span dir="ltr">@talarbavafa1</span> را ذخیره کنید، اما ارسال پیام تست همچنان VPN/Proxy سراسری یا دسترسی مستقیم سرور به بله می‌خواهد.
            </div>
            ...
            <form action={selectAction} className="grid gap-4 md:grid-cols-[1fr_1fr_auto]">
              <Field
                ...
                key={`chat-id-${chatId}`}
                defaultValue={chatId}
                placeholder="@my_channel یا -1001234567890 یا ble.ir/my_channel"
                dir="ltr"
              />
              <Field
                label="عنوان گفتگو"
            ...
            <form action={recentAction}>
              ...
              >
                <ListChecks size={18} />
                {isFetchingRecent ? "در حال جستجو..." : "جستجو در گفتگوهای اخیر بازو"}
              </button>
            </form>
        ...
        <div className="rounded-[1.75rem] border border-[#d8c08b]/62 bg-[#fff9ee]/94 p-5 shadow-[0_14px_44px_rgba(17,24,39,0.06)]">
          ...
          <div className="mt-3 grid gap-3 text-sm font-bold leading-7 text-[#6d5f49]">
            <div className="flex items-start gap-2">
              ...
              برای گفت‌وگوی عمومی، لینک ble.ir یا شناسه گفتگو را وارد کنید.
            </div>
            <div className="flex items-start gap-2">
              <Sparkles size={17} className="mt-1 shrink-0 text-[#17483f]" />
              برای گروه یا کانال خصوصی، بازو باید عضو یا مدیر باشد.
            </div>
            <div className="flex items-start gap-2">
              <Sparkles size={17} className="mt-1 shrink-0 text-[#17483f]" />
              برای جستجوی گفتگوهای اخیر، ابتدا به بازو پیام بدهید یا آن را به گروه اضافه کنید.
            </div>
          </div>
        </div>
      </aside>
